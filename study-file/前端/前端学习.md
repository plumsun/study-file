...
2.  :red_circle: 伪类选择器 ，通过 `.class 名` 来选中对应的标签元素，class 是 HTML 中所有标签的共同属性。
...
3.  :red_circle: 伪类选择器 ，通过 `#id`来选择对应属性 id 的标签元素，id 是 HTML 中所有标签的共同属性。
...
6.  :red_circle: 伪类选择器 ，通过指定标签的属性值来进行选择。
...
7.  :red_circle: 伪类和伪元素选择器
    ...
    -   伪元素选择器，主要针对的是标签中的内容，简单示例：`::pseudo-element-name`，以双引号开头。代码示例：
...
主要指的是要目标标签要修改的内容，可以是颜色、大小、位置、宽高等一些修饰属性。
